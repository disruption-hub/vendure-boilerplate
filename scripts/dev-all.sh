...
exec pnpm exec concurrently \
  ...
  "bash -c 'echo \"[Gateway] Waiting for services...\"; until curl -s -f http://localhost:3000/health > /dev/null; do sleep 1; done; until curl -s -f http://localhost:3005/ > /dev/null; do sleep 1; done; until curl -s -f http://localhost:3002/ > /dev/null; do sleep 1; done; echo \"[Gateway] Services ready, starting...\"; PORT=3004 pnpm --filter api-gateway start:dev'" \
  --kill-others \

...
import { ShieldCheck, Phone, Loader2, Lock, Wallet } from 'lucide-react';
...
export default function AdminLogin() {
    ...
    // Unified wallet kit is managed by the provider
    ...
    const handleBackendWalletLogin = async (connectedAddress: string) => {
        ...
        try {
            console.log('[AdminLogin] Attempting backend login for:', connectedAddress);
            ...
            if (!data.user?.roles?.includes('SYSTEM_ADMIN')) {
            ...
            console.error('[AdminLogin] Backend error:', err);
    ...
    const handleWalletLogin = async () => {
        ...
        try {
            const savedPublicKey = publicKey || localStorage.getItem('stellar_public_key');
            if (!savedPublicKey) {
                console.log('[AdminLogin] No wallet detected, opening modal...');
                const address = await connectWallet();
                if (address) {
                    console.log('[AdminLogin] Wallet connected in modal, finalizing login for:', address);
                    await handleBackendWalletLogin(address);
                }
            } else {
                console.log('[AdminLogin] Wallet already connected, triggering backend...');
                handleBackendWalletLogin(savedPublicKey);
            }
        } catch (err: any) {
            console.error('[AdminLogin] Connection error:', err);
    ...
    const handleSwitchWallet = async () => {
        setIsLoading(true);
        try {
            console.log('[AdminLogin] Switching wallet: disconnecting current...');
            await disconnectWallet();
            // Small delay to ensure state clears
            await new Promise(resolve => setTimeout(resolve, 300));
            console.log('[AdminLogin] Opening modal for new wallet selection...');
            const address = await connectWallet();
            if (address) {
                setIsWalletAttempt(true);
                await handleBackendWalletLogin(address);
            ...
            console.error('[AdminLogin] Switch wallet error:', err);
                ...
                <AnimatePresence mode="wait">
                    ...
                    {step === 'method' && (
                        <motion.div
                            ...
                            exit={{ opacity: 0, y: -20 }}
                            className="bg-slate-900 rounded-3xl p-8 border border-red-500/20"
                        >
                            <div className="text-center mb-8">
                                ...
                                <p className="text-slate-400 text-sm">System Administrator Login</p>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                {(publicKey || (typeof window !== 'undefined' && localStorage.getItem('stellar_public_key'))) && (
                                    <button
                                        ...
                                        disabled={isLoading}
                                        className="w-full text-center text-blue-400 text-xs mt-2 hover:underline disabled:opacity-50"
                                    >
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <div className="relative flex-1">
                                        ...
                                        <input
                                            ...
                                            required
                                            className="w-full pl-12 pr-4 py-3 rounded-xl border border-white/10 bg-black/20 text-white placeholder:text-slate-600 focus:border-red-500 focus:outline-none"
                                        />
                                    </div>
                                </div>
                    ...
                    {step === 'otp' && (
                        ...
                        >
                            <div className="text-center mb-8">
                                ...
                                <h1 className="text-2xl font-bold text-white mb-2">Security Challenge</h1>
                            ...
                            <form onSubmit={handleVerifyOtp} className="space-y-6">
                                <div>
                                    <input
                                        ...
                                        maxLength={6}
                                        required
                                        className="w-full text-center text-3xl font-mono tracking-[0.5em] py-4 rounded-xl border border-white/10 bg-black/20 text-white placeholder:text-slate-700 focus:border-red-500 focus:outline-none"
                                    />

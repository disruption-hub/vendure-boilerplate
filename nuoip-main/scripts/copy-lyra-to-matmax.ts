 ...
 * Run from packages/prisma: cd packages/prisma && npx ts-node ../../scripts/copy-lyra-to-matmax.ts
 * Or via Railway: railway run -- node -e "..."
 */
...
async function main() {
  ...
  if (!matmaxTenant) {
    ...
    tenants.forEach((t: { id: string; name: string; subdomain: string | null }) => console.log(`  - ${t.name} (${t.subdomain || 'no subdomain'}) [${t.id}]`))
    return
  }
  ...
  const currentSettings = (matmaxTenant.settings as any) || {}
  ...
  const verifySettings = verifyTenant?.settings as any

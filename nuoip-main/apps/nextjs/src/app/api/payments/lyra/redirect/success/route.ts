...
/**
 * API route to handle Lyra payment success redirects
...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/v1/payments/lyra/redirect/success${queryString ? `?${queryString}` : ''}`
    ...
    console.error('[Lyra Success Redirect] Error:', error)
    // Fallback redirect to success page
    const searchParams = request.nextUrl.searchParams.toString()
    const redirectUrl = `/payments/lyra/browser-success${searchParams ? `?${searchParams}` : ''}`
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.warn('[Lyra Success Redirect] Error parsing body:', parseError)
    }
    ...
    // Fetch from backend directly (backend returns HTML)
    const response = await fetch(`${BACKEND_URL}/api/v1/payments/lyra/redirect/success`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },

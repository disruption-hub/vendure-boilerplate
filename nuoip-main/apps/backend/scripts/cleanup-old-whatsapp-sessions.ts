...
async function deleteOldWhatsAppSessions() {
    ...
    console.log(`📊 Found ${oldContacts.length} old contacts with session ${oldSessionId}:`)
...
deleteOldWhatsAppSessions()
    .then(() => {
        console.log('\n✅ Cleanup complete!')
        console.log('👉 Now hard refresh the page to load messages with the correct session ID')
        process.exit(0)
    })
    .catch((error) => {
